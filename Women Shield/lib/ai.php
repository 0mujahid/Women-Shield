...
function generate_assistant_reply(string $message, array $context = []): string
{
    ...
    if ($reports !== [] && prompt_looks_like_location_lookup($message)) {
        ...
        if ($latestKnownLocation !== '') {
            return "I could not find any saved report for {$message}. Your recorded report location is {$latestKnownLocation}. If this is a different place, add a new report for it from the Reports page.";
        }
    }
    ...
    return 'I can help with emergency planning, report filing, safety map guidance, night travel precautions, and choosing the best next step from your dashboard. Tell me what situation you are in right now.';
}
